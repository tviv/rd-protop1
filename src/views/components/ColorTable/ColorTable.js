...
class ColorTable extends Component {
  ...
  getDetailRows = (rowInd) => {
    this.props.onExpand(rowInd).then((detailData) => {
  ...
  expandLoading =
    <div className="text-center" >
      <div className="spinner-border text-info" style={{height: "1rem", width:"1rem", borderBottomWidth:"0.15em", borderTopWidth:"0.15em", borderLeftWidth:"0.15em", borderRightWidth:"0.15em"}}/>
    </div>;
  ...
  getRows = (row, rowIndex)=>{
    ...
    const firstRow = (
      <tr key={rowIndex}>
        ...
        {/*<button  onClick={()=>this.handleExpand(rowIndex)}>*/}
          {this.isExpanded(rowIndex) && !this.detailMap.has(rowIndex)
            ? (this.expandLoading)
            : (<a className="btn btn-minimize" onClick={()=>this.handleExpand(rowIndex)} ><i className={this.isExpanded(rowIndex) ? "icon-arrow-down" : "icon-arrow-right"}/></a>)
          }
        ...
        {row.filter(x=>!x.hidden).map((col, index)=>
          <td id={col.cellId} className={'cell-cone ' + (col.cellId && this.props.cellId === col.cellId ? 'focus' : '')} style={{'background':col.background, ...styles.tableCell,... index >= 1 ? styles.tableCellValue : {}}} key={index} onClick={this.handleCellClick} >
            {col.label}
          </td>
        )}
      </tr>
    ...
    if(this.isExpanded(rowIndex)){
      ...
      if (detailData) {
        if (typeof detailData === 'string') {rows.push(<tr><td><br/></td><td className="text-danger">{detailData}</td></tr>); return rows}; //todo
        const detailRow = (
          detailData.rows.map((rowD, rowIndexD) =>
            <tr key={rowIndex * 1000 + rowIndexD}>
              <td className='cell-expand-button cell-expanded-empty'><br/></td>
                {rowD.map((col, index) =>
                <td className={'cell-cone'} style={{'background':col.background, ...styles.tableCell,... index >= 0 ? styles.tableExpandedCellValue : {}}} key={`dcell${rowIndex}_${index}`}>
                  {col.label}
                </td>
              )}
            </tr>
  ...
  render () {
    ...
    return (
        <table className="table table-bordered table-sm cell-cone">
          <thead>
          <tr>
            {this.isExpandable() && <th className='cell-expand-button'/>}
            {this.props.data && this.props.data.headerColumns.filter(x=>!x.hidden).map((item, index) => {
              return (<th key={index} style={{'background':item.background}}>{item.label}</th>)
              }
            )}
          </tr>
          </thead>
