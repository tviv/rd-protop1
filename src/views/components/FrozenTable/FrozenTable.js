...
class FrozenTable extends Component {
    ...
    componentDidMount() {
        ...
        if (this.table2Ref.rows && this.table2Ref.rows.length >= 2) {
            ...
            this.headRowsNumber = this.table2Ref.getElementsByTagName(
                'thead'
            )[0].rows.length;
    ...
    componentDidUpdate() {
        ...
        this.table1Ref.getElementsByTagName('thead')[0].innerHTML =
            mainTableThead[0].innerHTML;
        ...
        if (this.state.colNumbers) {
            ...
            for (let j = 0; j < cells2.length; ++j) {
                ...
                try {
                    ...
                    if (j < this.props.frozenCols) {
                        ...
                        cells1[j].width = `${
                            this.table2Ref.rows[1].cells[j].offsetWidth
                        }px`;
                    } else {
                        cells1[j].width = `${
                            this.table2Ref.rows[1].cells[j].offsetWidth
            ...
            if (this.state.scrollWidth === undefined)
                this.setState({
                    scrollWidth: this.table2Ref.offsetWidth + this.offsetLeft,
                });
        }
    }
    ...
    setFrozenFeatures(enabled) {
        ...
        if (enabled) {
            ...
            for (let j = 0; j < this.props.frozenCols; ++j) {
                if (this.table1Ref.rows[0] && this.table1Ref.rows[0].cells[0])
                    this.table1Ref.rows[0].cells[j].classList.remove(
                        'frozen-col'
                    );
                for (let i = 0; i < this.table2Ref.rows.length; ++i) {
                    let cell = this.table2Ref.rows[i].cells[j];
    ...
    render() {
        const childrenWithProps = React.Children.map(
            this.props.children,
            child => React.cloneElement(child, { onCng: this.tempHandle })
        );
            ...
            <div style={this.props.style}>
                <div className="sticky" style={{ zIndex: 111 }}>
                    <div style={{ position: 'relative' }}>
                        <div
                            ref={el => (this.div1Ref = el)}
                            style={{
                                overflowX: 'hidden',
                                paddingLeft: this.offsetLeft,
                            }}
                        >
                            <table
                                ref={el => (this.table1Ref = el)}
                ...
                <div style={{ position: 'relative' }}>
                    ...
                    <div
                        ref={el => (this.divTopScrollRef = el)}
                        style={{
                            overflowX: 'auto',
                            position: 'sticky',
                            bottom: 0,
                    ...
                    >
                        <div
                            style={{ height: 1, width: this.state.scrollWidth }}
                        />
                    </div>
                </div>
            </div>
