...
class DimensionSelect extends Component {
    ...
    componentDidMount() {
        ...
        this.refreshData();
        this.setHiddenOfSearch(
            this.props.getSelectedValues(this.props.hierarchyName) && true
        );
    }
    ...
    setHiddenOfSearch(value) {
        if (!this.props.alwaysSearch)
            this.myRef.node.className = classnames(
                { 'hide-search': value },
    ...
    setDefaultVaules(data, level = 0) {
        ...
        let selected =
            this.props.getSelectedValues(this.props.hierarchyName) || [];

        if (
            selected ||
            this.props.disableToLevel !==
                DimensionSelect.defaultProps.disableToLevel
        ) {
            data.forEach(x => {
    ...
    ungroup(data, ungroupElement, destinationElement) {
        ...
        const ungroupElementIndex = destinationElement.lastIndexOf(
            ungroupElement
        ); //now for all our cases we can use it without recurse
    ...
    handleChange = (currentNode, selectedNodes) => {
        ...
        if (this.props.onChange) {
            this.props.onChange(
                this.props.hierarchyName,
                selectedNodes.map(x => x.value)
            );
        }
    ...
    render() {
        return (
            <DropdownTreeSelect
                ...
                noMatchesText="нет соответствия"
                placeholderText={this.state.placeholderText}
                data={this.state.data}
                onChange={this.handleChange}
                onAction={this.onAction}
                onNodeToggle={this.onNodeToggle}
                className={'dim-select ' + this.tempUniqClassName}
