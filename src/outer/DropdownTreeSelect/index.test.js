...
test('notifies on action', t => {
    ...
    const wrapper = shallow(
        <DropdownTreeSelect data={tree} onAction={handler} />
    );
...
test('notifies on node toggle', t => {
    ...
    const { tree } = t.context;
    const wrapper = shallow(
        <DropdownTreeSelect data={tree} onNodeToggle={handler} />
    );
...
test('notifies on checkbox change', t => {
    ...
    const { tree } = t.context;
    const wrapper = shallow(
        <DropdownTreeSelect data={tree} onChange={handler} />
    );
    wrapper.instance().onCheckboxChange(node0._id, true);
    t.true(
        handler.calledWithExactly({ ...node0, checked: true }, [
            { ...node0, checked: true },
        ])
    );
});
...
test('clears input onChange for clearSearchOnChange', t => {
    const { tree } = t.context;
    const wrapper = mount(
        <DropdownTreeSelect data={tree} clearSearchOnChange />
    );
    wrapper.instance().onInputChange('it');
    wrapper.instance().onCheckboxChange(node0._id, true);
    t.false(wrapper.state().searchModeOn);
