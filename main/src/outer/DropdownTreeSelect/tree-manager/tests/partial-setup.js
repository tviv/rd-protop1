...
export const assertTreeInExpectedState = (t, manager, expected) => {
    ...
    checked.forEach(c =>
        t.truthy(
            manager.getNodeById(c).checked,
            `Expected node ${c} to be in checked state`
        )
    );
    partial.forEach(c =>
        t.truthy(
            manager.getNodeById(c).partial,
            `Expected node ${c} to be in partial state`
        )
    );
    unchecked.forEach(c =>
        t.falsy(
            manager.getNodeById(c).checked,
            `Expected node ${c} to be in unchecked state`
        )
    );
    nonPartial.forEach(c =>
        t.falsy(
            manager.getNodeById(c).partial,
            `Expected node ${c} to be in non-partial state`
        )
    );
};
