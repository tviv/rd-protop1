...
class TreeManager {
    ...
    togglePreviousChecked(id) {
        ...
        if (prevChecked && prevChecked !== id)
            this.getNodeById(prevChecked).checked = false;
    ...
    partialCheckParents(node) {
        ...
        while (parent) {
            const next = this.getNodeById(parent);
            next.checked = next._children.every(
                c => this.getNodeById(c).checked
            );
            next.partial = getPartialState(
                next,
                '_children',
    ...
    collapseChildren(node) {
        ...
        if (!isEmpty(node._children)) {
            node._children.forEach(c =>
                this.collapseChildren(this.getNodeById(c))
            );
        }
    }
    ...
    getTags() {
        ...
        const markSubTreeVisited = node => {
            ...
            if (!isEmpty(node._children))
                node._children.forEach(c =>
                    markSubTreeVisited(this.getNodeById(c))
                );
        };
