...
const data2 = {
    ...
    rows: (() => {
        ...
        for (let i = 0; i < 3; i++) {
            res.push([]);
            for (let j = 0; j < colNums2; j++) {
                res[i].push({ label: 'e' + j.toString() });
            }
        }
        return res;
    })(),
    ...
    getRows = (row, rowIndex) => {
        ...
        const firstRow = (
            <tr key={rowIndex}>
                ...
                {/*</td>*/}
                {row.map((col, index) => (
                    <td key={`cell${rowIndex}_${index}`}>{col.label}</td>
                ))}
            </tr>
        );
        ...
        if (this.isExpanded(rowIndex)) {
            const detailRow = data2.rows.map((row2, rowIndex2) => (
